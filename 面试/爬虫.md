...
3.native层混淆，代码混淆与控制流平坦化，无法直接F5，或者清楚分析出代码逻辑，代码块动态跳转计算
4.asset获取PIC数据，动态生成的，多层加密的json数据包
...
设备指纹与唯一性标识
1.多维信息采集：硬件信息，系统属性，陀螺仪，mac，硬件id，cup
PIC数据获取KEY IV
AES加密json数据
...
- 数据/设备环境检测的值 debugger frida xposed检测之类 RC4
- 设备指纹信息 AES
- 时间戳
- 加密密钥（从资源文件动态解密获得）
...
SDK提供的核心接口：
- initialize：初始化SDK
- avmpSign：对输入数据进行签名，生成wToken

工作流程：
- sdk初始化：采集设备信息
- 环境检测和设备指纹生成：采集大量设备与环境参数，生成唯一设备指纹

- 签名与加密：使用appkey Base64+AES  对数据进行加密签名，在发出请求时生成wToken
- 网络请求：将wToken与业务数据一同发送到服务器
- 风险识别：服务器解析wToken，进行风控

逆向工作：
- 字符串加密
- 代码混淆，控制流扁平化，vm，间接跳转
- bytecode藏在图片里面
- 反调试反Hook：调试器，框架，模拟器，多开，代码完整性校验dladdr函数，可以获得某地址符号信息

加密与签名流程：
- appkey需经过Base64解码和AES解密后才能使用
- 设备信息先经自定义算法处理，再使用AES加密

*{密钥索引}{加密后的设备信息}&{原始请求体数据}&{最终签名值}*
wToken 由以下关键部分组合而成：

1. ​**密钥索引（Key Index）​**​：例如 `KIUR_`，用于标识加密密钥。
    
2. ​**加密后的设备信息**​：原始设备信息经过异或加密、压缩、AES加密（密钥为特定MD5值的前16字节）、Base64编码后得到的字符串。
    
3. ​**原始请求体数据**​：即需要签名的 `data`参数内容。
    
4. ​**最终签名值**​：对“组合后的数据（原始请求体 + '&' + 密钥索引 + 加密后的设备信息）”计算SHA1哈希值，该哈希值再经过虚拟机（VM）内复杂的循环加密逻辑（包括异或、移位等操作）处理后得到的一串字符。
...
#### HTTP协议：
http方法：
- GET：请求资源，参数通过url传递，可被缓存，不应修改服务器状态
- POST：向指向资源提交数据，数据在请求体中被传输，不可被缓存，可能修改服务器状态
- DELETE：删除指定资源
- PUT：替换目标资源的所有当前表示
- HEAD：获取资源的元数据
- PATCH：对资源进行部分修改
...
Connection：
3.请求体：
ex：username=admin&password=123456
...
协议：//[username:password@]host[:port]/path【?query】【 # fragment】
...
URI：统一资源标识符，URL+URN
URN：统一资源名称，urn:NID:NSS

#### web数据结构
##### HTML标签结构
1.div和span标签
div 块级标签，分割文档，占据一行，前后自动换行
span 内联容器元素，对文档小部分内容进行标记或样式化
...
- [target=  blank ]：选择target="blank"的所有元su
...
##### JSON数据结构
python与json数据转换：json.dumps();json.load()
...
#### Scapy问题
增量爬取：保存上一次状态，本次如果不在上次状态，视为增量
signals模块通知事件发生

scrapy-redis：基于redis数据库，支持分布策略，共享队列，共享指纹

分布式解决的问题：ip，带宽，cpu，io密集

反向代理：给服务器代理，想要访问服务器，先访问代理服务器
- 隐藏真实IP
- 负载均衡
- 提高访问速度
- 提供安全保障
...
分布式策略：
- slaver从master拿任务进行数据抓取，同时产生新任务给master，master有redis数据库，对提交的任务去重，加入任务
- master跑一个程序去生成任务，master负责生产任务，并去重，加入队列，slaver只负责拿来爬
- master只有一个集合，只有查询作用；
...
- 中间件设计
缺点：
- 基于python，扩展性差，接入高性能库，扩展性差
- 基于twisted框架，对异常处理的捕获，不会停止阻塞任务，有些问题难以察觉
#### Scrapy
##### Scrapy
###### 架构
1. Engine：处理整个系统的数据流和事件
2. Item：抽象的数据结构，定义了爬取结果的数据结构
3. Scheduler：调度器，接受Engine发过来的Request，并加入队列，也可以将Request发回给Engine供Donwloader执行；维护Request调度逻辑，先进先出，先进后出，优先级进出
4. Spiders：每个spider定义了站点的爬取逻辑和页面解析规则，主要负责解析响应并生成Item和新的请求，发给Engine处理
5. Downloader：完成向服务器发送请求，然后拿到响应的过程，响应发给Engine处理
6. Item PipeLines：项目管道，每个Itempipline负责处理由Spider从页面抽取的Item，做一些数据清洗，验证，存储的工作
7. Downloader Middlewares：位于Engine和Downloader之间的Hook框架
8. Spider Middlewares：Engine和Spiders之间的部分，负责Spider和Engine之间的Item，请求和响应处理

###### 数据流
包括Item，Request，Response
 1. Engine根据爬取的站点，找到对应Spider，生成需要的Request，发给Engine
 2. Engine把Request交给Scheduler调度
 3. Engine向Scheduler索取下一个要处理的Request，Scheduler根据调度逻辑发给Engine
 4. Engine再将发来的Request转发给Downloader下载，这个过程会被Downloader中间件处理
 5. Downloader得到对应Response，返回给Engine，也被Downloader中间件处理
 6. Engine把Response发给Spider，这个过程被Spider中间件处理
 7. Spider处理Response，解析内容，产生一个或者多个爬取结果Item或者后续要爬取的页面的任意数量Request
 8. Engine将Spider发回的Item转发给ItemPipelines进行数据处理或存储，将Request发给Scheduler调度
...
- **yield item/request**  
    → 把结果交给 pipeline 或 scheduler。
    
- **Scrapy Engine 调度下一轮请求**。
...
class BookPipeline:
    ...
    def open_spider(self, spider):
        # 当 spider 打开时执行
        print(f"{spider.name} 开始爬虫")

    def close_spider(self, spider):
        # 当 spider 关闭时执行
        print(f"{spider.name} 关闭爬虫")
...
- 对Response统一处理
- 处理重定向或状态码
- 模拟反爬（动态修改headers或者解析验证码）
3.SpiderMiddleware
- 在Spider收到Response之前，或发送Request之后处理
- 过滤不要的Response
- 修改Spider输出的Item或者Request
###### Middleware 可重写的方法

|方法|类型|说明|
|---|---|---|
|`process_request(self, request, spider)`|Downloader|请求发出前处理 request|
|`process_response(self, request, response, spider)`|Downloader|响应返回后处理 response|
|`process_exception(self, request, exception, spider)`|Downloader|处理请求异常|
|`process_spider_input(self, response, spider)`|Spider|Spider 接收 response 前|
|`process_spider_output(self, response, result, spider)`|Spider|Spider 输出 item 或 request 后|
|`process_spider_exception(self, response, exception, spider)`|Spider|Spider 抛异常时|
|`from_crawler(cls, crawler)`|两类均可|用于从 settings 或 signals 初始化中间件|
...
- 访问限制和请求伪造
	- 代理池，速率设置
	- 请求伪造，useragent，TLS指纹，Referer，Accept-Language
	- 会话持久化：周期性更新
	- 人机验证通过
- JS动态
	- 接口逆向
...
- 分布式队列
- 请求去重
	- BloomFilter：巨大的bit_array,已经请求过的url，经过hash，取模，得到一个索引，这个过程可以重复多次，为保证不同，每次添加一个种子hash，然后拼接；在这多个索引下，数值为1，这就构成了这个请求的一个身份证，比如在bit[243472]，在bit[43424]。。。等几处为1，那么如果想要查询的请求，经过操作，也是那几位为1，那么重复
	- 滑动窗口模式：由于爬得越多，填充率越高，所以不断更新bloom，
	假设窗口长度 = 1 天，维护 3 个 Bloom：查询重复的时候三个bloom都要查
...
#### 爬虫八股
1. 使用过的爬虫框架
	1. scrapy，提取结构性数据编写的应用框架，应用在数据挖掘，信息处理或者存储历史数据等程序中；
	2. BeautifulSoup，urllib，requests，re，aiohttp，selenimu，BS可以从HTML或者XML中提取数据
2. 三次握手，四次挥手
	1. tcp三次握手四次挥手：需要三次是因为担心服务器接收到已失效请求
		 - SYN=1
		 - SYN=1-ACK=1
		 - ACK=1
		挥手：全双工的，所以每个方向都需要确定关闭
		 ...
		 - ACK=1
3. 爬虫是多进程好还是多线程好
	1. IO密集型的代码，多线程效率高；CPU密集型，多进程；
	2. 实际情况，要考虑网速和响应问题，设置多线程和多进程
4. 常见反爬虫和应对方法
	1. 信息反爬，Headers
	2. 用户行为，IP频率，账户频率--IP池，账户池
	3. 动态渲染加载方式，ajax请求，或者js生成，js加密请求参数----selenium或者js逆向
5. 需要登录的网页，如何解决同时限制IP，Cookie，session
	1. 代理池，Cookie动态更新监控Cookie过期时间，
6. 验证码解决
	1. tesserocr
	2. selenimu解决滑动--使用合理滑动曲线
	3. 第三方库识别
	4. 深度学习训练模型
7. 爬虫数据存储
	1.  MongoDB
...
9. scrapy
	...
	4. 中间件设计，可以灵活完成爬取任务
10. 浅拷贝，只是引用
11. 装饰器增加函数功能，本质上是高阶函数，日志记录，性能测试，权限验证，缓存实现
12. 进程独立空间，线程CPU调度单位
13. scrapy-redis实现了任务队列共享和去重，使得支持分布式爬取
14. IP池-定期检测-大小管理-来源轮换-黑名单检测
15. Cookie过期处理：
	1. 异常类捕获
	2. 切换账号或者刷新cookie
16. 动态加载，时效性高处理
	1. selenium+headless模式的chrome/firefox
	2. webDriverwait
	3. js逆向
17. 代理IP类型：
	1. 透明代理，没用
	2. 普通匿名代理，隐藏了IP但能被识别是代理
	3. 高匿名代理，首选
	4. 检测代理健康机制
18. Redis基于内存存储，但是会通过定期的内存快照存留数据
	1. AOF日志持久化-记录每个写操作命令，以追加的方式写入日志文件
	2. RDB快照持久化，在指定时间间隔，将内存数据集中生成快照并保存到磁盘上的二进制文件中；
#### APP自动化
UIAutomator
...
###### APP加固：伪造请求构造不可得
- 目的：不能完全模拟app的请求，获得服务器响应
- 涉及代码混淆，反调试，环境检测，抓包对抗，完整性检查
...
#### API逆向和APP数据抓取
[岗位可能方向](https://www.cnblogs.com/andy0816/p/15364761.html)

https://www.cnblogs.com/GKLBB/p/19055535

https://cn-sec.com/archives/4273894.html

Token机制：
1. Access Token 每次请求都带
2. Refresh Token 用来自动换新Access Token
###### APP数据抓取：
抓包
分析API请求，定位
数据库/密钥 本地数据抓取
...
通信模式：
1.普通请求类：HTTPS+短链接+Token鉴权

2.持续通信类：WebSocket/MQTT/TCP长连接
...
定位网络层找拦截器逻辑：加密，签名，添加通用头往往在这里完成：
	搜Interceptor，okhttp3
	frida hook okhttp等拦截器
	动态验证frida hook

Retrofit框架识别利用：
	基于OkHttp的RESTful网络请求库，通过接口和注释来定义请求，极大简化代码
	@POST，@GET ，/login等注解
...
2. 搜索URL 路径片段
3. 搜索参数 搜索参数关键词
4. 查存储  本地存储数据-sf，db，sdcard
5. 挖拦截器  网络库层面分析
6. 动态验证 frida hook，拦截 拦截器
...
##### 手脱壳
1. 内存dump
2. hook脱壳，在关键的dlopen，dexfileparese函数出截取buffer
3. AOSP脱壳
4. 主动触发脱壳，主动调用，内存dump
5. 静态分析还原
