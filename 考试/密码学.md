...
2-2KG结构，要求是什么
LFSR+非线性组合，或者是NLFSR（分析复杂），最常用是第一个
...
4-1 hash函数的功能？实现方法？
完成消息认证，数据完整性，数字签名，单向函数，随机数生成，压缩信息

利用分组密码设计，专门设计的方法，利用模运算实现的方法（扩散，置换，混淆扩散，反馈压缩
...
4-3md结构
结构简单高效，具有压缩函数的强抗碰撞性
面临的攻击：多碰撞攻击，长消息的第二原想攻击，herding攻击
...
**4-11 Hash****函数主要应用在哪些方面？**


检测数据完整性、实现消息认证、消息认证码、认证加密、数字签名、实现单向函数、实现随机数发生器等 。



5-1 Diffie-Hellman公钥思想
每个用户有两个密钥，一个公开，一个保密，公开的加密，保密的解密
这样无需像对称密码那样事先建立共享密钥，只要公开接收方的公钥，任何人都可以给他传递密文

5-2公钥密码应用
速度较慢，规模较大，不适合大量数据加密，用于传递对称密钥和数字签名。

5-3实现方式
RSA ElGamal EC-ElGamal
基于大数分解难题，基于乘法群离散对数算法，基于椭圆曲线加法群离散对数算法
都借用数论中的经典计算难题问题设计实现
...
RSA签名算法，基于离散对数问题的ELGamal签名算法，基于椭圆曲线的加法群离散对数ECDSA算法
...
6-1密钥传递和保存的方法
加密传递，可靠信道传递，人工传递；采用密码技术传递对称密钥，密钥传输和密钥协商
物理介质存储，加密存储，分离存储

6-2密钥生命周期
生产，分发，存储，使用，更新，归档，撤销，备份，恢复，销毁

6-3KEM
KEM是指密钥封装机制，将一个对称密钥用公钥加密技术进行传输，以便在解决对称密钥建立问题，实现方便，安全传输对称密钥

三个算法组成：
密钥生成算法，输入安全参数k，输出一对公私钥
封装算法：输入公钥p，输出对称密钥和密文
解封装算法：输入私钥s，密文f，输出一个密钥k


6-4在diffile-hellman密钥协商协议中
...
（1） 假冒攻击：攻击者使用伪造的身份信息或证件进行身份认证，以获取未经授权的访问权限或敏感信息。

（2）重放攻击：攻击者截获合法通信信息后，再次发送该通信信息，以冒充合法用户；

（3）交错攻击：攻击者把前面一次或多次执行协议时的信息有选择地组合在一起，伪装成他人将这些信息进行传送，以达到攻击的目的。

（4）反射攻击：正在执行的协议中，攻击者冒充通信双方中的某一方，把另一方发送过来的信息再发送回去。